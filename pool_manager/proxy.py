...
_config = {
    ...
    "default_model": "deepseek-v4-flash",     # 安全锁定：默认 provider 强制使用此模型
...
async def _do_proxy_inner(request: Request, body: dict, client_ip: str) -> Response:
    ...
    # 🛡️ 安全锁定：默认 provider 强制使用 default_model
    if primary == _config["default_provider"]:
        dm = _config.get("default_model", "deepseek-v4-flash")
        if model != dm:
            model = dm
            body = dict(body)
            body["model"] = dm
