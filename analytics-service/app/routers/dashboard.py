...
async def get_dashboard():
    ...
    try:
        ...
        # 话痨榜（7天最长对话深度）
        c.execute("SELECT COALESCE(NULLIF(username,''), user_id) AS display_name, MAX(msg_count) FROM analytics.proxy_api_calls WHERE created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai' - INTERVAL '7 days' AND msg_count > 0 GROUP BY display_name ORDER BY MAX(msg_count) DESC LIMIT 10")
