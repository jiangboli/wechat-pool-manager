...
async def get_dashboard():
    ...
    try:
        ...
        # ── 对话深度排行（7天最长对话轮次）──
        c.execute(f"SELECT COALESCE(NULLIF(username,''), user_id) AS display_name, MAX(msg_count) FROM analytics.proxy_api_calls WHERE created_at >= {BJ_START} - INTERVAL '7 days' AND msg_count > 0 GROUP BY display_name ORDER BY MAX(msg_count) DESC LIMIT 10")
        data["top_chatty"] = [[r[0], r[1]] for r in c.fetchall()]

        # ── 小时热力图（7天，北京时）──
        c.execute("SELECT to_char(created_at + INTERVAL '8 hours', 'MM-DD') as day, extract(hour from created_at + INTERVAL '8 hours')::int as hour, count(*) FROM analytics.proxy_api_calls WHERE created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai' - INTERVAL '6 days' GROUP BY 1, 2 ORDER BY 1, 2")
        rows = c.fetchall()
